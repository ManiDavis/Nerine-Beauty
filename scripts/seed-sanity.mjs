...
const siteSettings = {
  ...
  aboutBody: "Michelle has dedicated her career to helping clients look and feel their very best. Based at La Sabri in the Vale, she brings warmth, professionalism and an exceptional eye for detail to every single appointment.",
  aboutBody2: "Over more than 20 years, Michelle has built lasting relationships with hundreds of Guernsey clients. Her approach is deeply personal: she takes the time to understand your unique skin, lifestyle and goals, then selects the treatments and products that will truly make a difference.",
...
async function seed() {
  console.log('🌱 Seeding Sanity project 7bdb2ubj...\n')
  ...
  try {
    ...
    console.log(`✅ Created/updated ${mutations.length} documents:`)
    console.log(`   • ${treatments.length} treatments`)
    console.log(`   • ${testimonials.length} testimonials`)
    console.log('   • 1 site settings document\n')
    console.log('🎉 Done! Open https://7bdb2ubj.sanity.studio or your /studio route to see the content.')
  } catch (err) {
    console.error('❌ Error:', err.message)
    if (err.statusCode === 403) {
      console.log('\nThe token does not have write permission.')
      console.log('Go to sanity.io/manage → your project → API → Tokens')
      console.log('and create an Editor token, then re-run with that token.')
    }
  }
}
